...
2. **В коде уже есть ручной C++.** [encoder.hpp](../../bsp/encoder/encoder.hpp) — это
...
`bsp/encoder/` сейчас — это 12 typedef'ов указателей на функции, две struct-таблицы
...
| Модуль | Что даёт |
|---|---|
| **`bsp/encoder/`** | Прямая замена ручной vtable на абстрактный класс. Минус ~100 строк бойлерплейта, плюс проверки компилятора. **Лучший кандидат на пилот.** |
...
- **Есть несколько реализаций одной абстракции.** Разные энкодеры, разные датчики тока,
  разные протоколы связи. Ровно ваш случай с `bsp/encoder/`.
...
7. **Пилотный модуль.** Рекомендация — `bsp/encoder/`: изолирован, имеет ровно одну
